...
class Upcoming extends React.Component{
  ...
  componentDidMount(){
    fetch(`${this.YOUTUBE_SEARCH_API}?part=snippet&eventType=upcoming&type=video&maxResults=50&order=relevance&safeSearch=none&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`)
      .then((res) => res.json())
      .then((json) => {
        this.setState({
          data: json
  ...
  componentDidUpdate(){
    ...
    if(this.CATEGORY === 1){
      fetch(`${this.YOUTUBE_SEARCH_API}?part=snippet&eventType=upcoming&type=video&maxResults=50&order=relevance&safeSearch=none&topicId=/m/04rlf&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`)
        .then((res) => res.json())
        .then((json) => {
          this.setState({
            data: json
    ...
    if(this.CATEGORY === 2){
      fetch(`${this.YOUTUBE_SEARCH_API}?part=snippet&eventType=upcoming&type=video&maxResults=50&order=relevance&safeSearch=none&topicId=/m/0bzvm2&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`)
        .then((res) => res.json())
        .then((json) => {
          this.setState({
            data: json
    ...
    if(this.CATEGORY === 3){
      fetch(`${this.YOUTUBE_SEARCH_API}?part=snippet&eventType=upcoming&type=video&maxResults=50&order=relevance&safeSearch=none&topicId=/m/06ntj&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`)
        .then((res) => res.json())
        .then((json) => {
          this.setState({
            data: json
    ...
    if(this.CATEGORY === 4){
      fetch(`${this.YOUTUBE_SEARCH_API}?part=snippet&eventType=upcoming&type=video&maxResults=50&order=relevance&safeSearch=none&topicId=/m/02jjt&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`)
        .then((res) => res.json())
        .then((json) => {
          this.setState({
            data: json
    ...
    if(this.CATEGORY === 5){
      fetch(`${this.YOUTUBE_SEARCH_API}?part=snippet&eventType=upcoming&type=video&maxResults=50&order=relevance&safeSearch=none&topicId=/m/019_rr&key=${process.env.REACT_APP_YOUTUBE_API_KEY}`)
        .then((res) => res.json())
        .then((json) => {
          this.setState({
            data: json
